...
def _build_issue_url(batch: list, skipped: int, total_remaining: int) -> str:
    ...
    more = total_remaining - len(batch)
    footer = f"\n\n*+{more} more — run Propose again after merging this issue.*" if more else ""
    body = (
        f"## New tracker domains from Mailtrack Hunter\n\n"
        f"{len(batch)} of {total_remaining} pending domain(s) to add to `rules/rules.json`"
        + (f" ({skipped} already covered by existing rules, skipped)" if skipped else "")
        + f":\n\n"
    ...
    title = f"Add {len(batch)} new tracker rule(s) from Mailtrack Hunter"
    return (
...
async def export_github_issue():
    ...
    # Build the largest batch that fits within the GitHub URL limit
    batch = []
    for r in new_rows:
        candidate = list(batch) + [r]
        if len(_build_issue_url(candidate, skipped, len(new_rows))) <= _GITHUB_URL_LIMIT:
            batch.append(r)
        else:
            break

    # Always include at least one candidate
    if not batch:
        batch = [new_rows[0]]

    issue_url = _build_issue_url(batch, skipped, len(new_rows))
    ids = [r["id"] for r in batch]
    remaining_after = len(new_rows) - len(batch)
